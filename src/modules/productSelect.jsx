...
class ProductSelect extends Component {
    ...
    async pdfUploadOnChange(event){
        ...
        if(selectedFile.size < 5000000 && regex.test(selectedFile.name)){
            ...
            await axios.post(process.env.REACT_APP_backendAPI+"/uploadpdf",data);
            this.setState({
                errorMessageUpload : false,
    ...
    async checkDomain(){
        ...
        if(valid){
            ...
            const {data} = await axios.post(process.env.REACT_APP_backendAPI+"/checkdomain",{"domainName" : domain});
            this.setState({
