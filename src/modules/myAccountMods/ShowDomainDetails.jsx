...
class ShowDomainDetails extends Component {
    ...
    async handleConfirmBtn(){
        ...
        try {
            const {data : response} = await axios.post('http://localhost:4000/updateDomainRegistrar',{
    ...
    async handleCancelBtn(){
        try {
            const {data : response} = await axios.post('http://localhost:4000/showDomainNames',{
                "token" : sessionStorage.getItem("token")
            }); 
            console.log(response);
            this.setState({
