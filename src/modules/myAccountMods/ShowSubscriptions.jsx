...
class ShowSubscriptions extends Component {
    ...
    async componentDidMount(){  
        ...
        try {
            ...
            const {data : response} = await axios.post('http://localhost:4000/getUserSubscriptions',{"token" : token}); 
    ...
    async handleYes(){
        ...
        const {data : response} = await axios.post('http://localhost:4000/deleteSubscription',{
            "token"         : token,
            "deleteIndex"   : this.state.showYesNo,
            "subscriptionId": this.state.subscriptions[this.state.showYesNo].subscriptionId
        }); 
...
export default ShowSubscriptions;
    ...
    //     try {
    //         token = sessionStorage.getItem("token");
    //         const {data : response} = await axios.post('http://localhost:4000/getSubscriptions',{"token" : token}); 
    //         console.log(response);
